...
def display():
    # Título de la página
    st.markdown("<h1 style='text-align: center;'>Día 8</h1>", unsafe_allow_html=True)
    st.markdown("<br><br>", unsafe_allow_html=True)

    # Introducción del día
    st.markdown("""
        <div style='text-align: justify; font-size: 18px;'>
            Este es el mapa de la ruta correspondiente al Día 8 de nuestro viaje. 
            Las siguientes paradas reflejan los momentos y lugares que visitamos el octavo día.
        </div>
        <br><br>
    ...
    # Filtrar filas con el día 8 (correspondiente al número 9 en la columna 'dia')
    df_dia_8 = df[df['dia'] == 9].sort_values(by='date_time').reset_index(drop=True)

    # Crear un mapa centrado en Manhattan
    map_center = [41.67941, -85.53026]
    map = folium.Map(location=map_center, zoom_start=8)

    # Añadir marcadores al mapa
    coordinates = []
    for idx, row in df_dia_8.iterrows():
        if row['foto'] == 'SI':
    ...
    for segment in segmentos:
        start_idx = get_index(df_dia_8, segment['start'])
        end_idx = get_index(df_dia_8, segment['end'])
